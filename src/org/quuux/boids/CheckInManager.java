        ...
        try {
            ...
            if(response.has("status") && 
               response.getString("status").equals("ok")) {  
                if (BuildConfig.DEBUG)Log.d(TAG, "Collector accepted " + key + " checkin");
                CheckInManager.setCheckedIn(key);
            } else {
                if (BuildConfig.DEBUG)Log.d(TAG, "Collector did not accept " + key + " checkin");
            }

        } catch(Exception e) {
            if (BuildConfig.DEBUG)Log.d(TAG, "Could not send check in: " + e);
        }            
    }
}
...
class CheckInManager {
    ...
    private static void sendDeviceProfile() {
        ...
        try { 
            ...
            if (BuildConfig.DEBUG)Log.d(TAG, "Could not send device check in: " + e);
        }
    }
    ...
    public static void sendGLProfile() {
        ...
        try {
            ...
            if (BuildConfig.DEBUG)Log.d(TAG, "Could not send GL check in: " + e);
        }
    ...
    private static void checkIn(String key, JSONObject data) {
        if (BuildConfig.DEBUG)Log.d(TAG, "Sending " + key + " check in");
